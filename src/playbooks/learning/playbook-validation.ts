import type { EvolvedPlaybook, PlaybookReviewStatus } from './types';
import { APPROVED_THRESHOLD } from './config';
...
const ALLOWED_STATUS_TRANSITIONS: Record<PlaybookReviewStatus, PlaybookReviewStatus[]> = {
  draft: ['pending', 'approved', 'archived', 'suspended'],
...
export function validatePlaybookShape(playbook: EvolvedPlaybook): PlaybookValidationResult {
  ...
  if (playbook.reviewStatus === 'approved' && playbook.confidence < APPROVED_THRESHOLD) {
    issues.push({
      field: 'reviewStatus',
      message: `approved status typically requires confidence >= ${APPROVED_THRESHOLD}`,
      severity: 'warning',
    });
  }
