/**
 * v2 Goal Manager DLQ Replay API
 * POST /api/v2/goal-manager/replay
 */
  ...
  try {
    if (!isAuthorized(request)) {
      return NextResponse.json(
        { error: 'Unauthorized: replay requires admin x-api-key', code: 'UNAUTHORIZED' },
        { status: 401 }
      );
    }
    ...
    if (!goalId) {
      return NextResponse.json({ error: 'goalId is required', code: 'VALIDATION_ERROR' }, { status: 400 });
    }

    const result = await replayGoalManagerDlq(goalId);
    if (!result.replayed) {
      return NextResponse.json({ data: result, meta: meta() }, { status: 404 });
    }
