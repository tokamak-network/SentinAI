...
describe('/api/marketplace/incident-summary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
      ...
      merchantAllowlist: [
        {
          ...
          resources: ['/api/marketplace/incident-summary'],
  ...
  it('returns 402 requirements for incident-summary', async () => {
    const { GET } = await import('@/app/api/marketplace/incident-summary/route');
    const response = await GET(new NextRequest('https://sentinai.example.com/api/marketplace/incident-summary'));
    const body = await response.json();

    expect(response.status).toBe(402);
    expect(body.paymentRequirements.amount).toBe('150000000000000000');
    expect(body.paymentRequirements.resource).toBe('/api/marketplace/incident-summary');
    expect(body.paymentRequirements.merchant).toBe('0x5555555555555555555555555555555555555555');
  });
});
