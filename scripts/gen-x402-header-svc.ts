...
const svc = process.argv[2] || 'sequencer-health';
const amount = process.argv[3] || '100000000000000000';
...
const resource = `/api/agent-marketplace/${svc}`;
...
async function main() {
  ...
  const sig = await client.signTypedData({
    domain: { name: 'SentinAI x402 TON Facilitator', version: '1', chainId: sepolia.id, verifyingContract: '0xe3Dd67941371fdC08685f3f93408DaB55b1E7581' },
  ...
  process.stdout.write(Buffer.from(JSON.stringify({
    ...
    payload: { buyer: account.address, merchant: msg.merchant, asset: msg.asset,
      amount, resource, nonce, validAfter: String(msg.validAfter),
      validBefore: String(msg.validBefore), signature: sig },
  })).toString('base64'));
}
