...
download_op_deployer() {
    ...
    tar -xzf op-deployer.tar.gz -C "$extract_dir"
    ...
    local extracted_dir
    extracted_dir=$(find "$extract_dir" -name "op-deployer-*" -type d | head -1)

    if [ -n "$extracted_dir" ] && [ -f "$extracted_dir/op-deployer" ]; then
        binary_path="$extracted_dir/op-deployer"
    elif [ -f "$extract_dir/op-deployer" ]; then
        binary_path="$extract_dir/op-deployer"
    elif [ -f "$extract_dir/op-deployer-$platform" ]; then
        binary_path="$extract_dir/op-deployer-$platform"
    else
        # Look for it anywhere (macOS compatible)
        binary_path=$(find "$extract_dir" -name "op-deployer*" -type f -perm -111 | head -1)
    fi
