...
ensure_host_execution() {
    if [ -f "/.dockerenv" ]; then
        log_error "This script must run on the host, not inside a container."
        exit 1
    fi
}
...
EOF
    ...
    # Keep docker-compose variable substitution values in the root .env.
    upsert_env_var "$ROLLUP_DIR/.env" "ROLLUP_RPC" "http://op-node:8547"
    upsert_env_var "$ROLLUP_DIR/.env" "PROPOSER_ADDRESS" "$PROPOSER_ADDRESS"
    upsert_env_var "$ROLLUP_DIR/.env" "CHALLENGER_ADDRESS" "$CHALLENGER_ADDRESS"
    upsert_env_var "$ROLLUP_DIR/.env" "GAME_FACTORY_ADDRESS" "$GAME_FACTORY_ADDRESS"
