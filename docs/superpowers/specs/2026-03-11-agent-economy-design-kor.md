...
- **facilitator 정의 EIP-712 authorization**: TON facilitator가 `approve + transferFrom` 정산 전에 검증하는 구매 서명 의도
...
4. 결제 후 재시도
   에이전트가 0.1 TON에 대한 facilitator 전용 EIP-712 `PaymentAuthorization` 서명
   에이전트 → GET /api/marketplace/sequencer-health
              X-PAYMENT: <base64 PaymentPayload>

5. 검증 후 응답
   payment-verifier.ts가 facilitator authorization을 검증하고, same-app facilitator에 TON 정산을 요청한 뒤, signed settlement receipt를 검증
...
src/lib/marketplace/
  ...
  payment-verifier.ts   — TON facilitator authorization 검증 + facilitator 호출
