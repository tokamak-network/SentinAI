...
Full report: [`docs/guide/testing/algorithm-effectiveness-evaluation.md`](guide/testing/algorithm-effectiveness-evaluation.md)
...
## Detailed Review (2026-03-04 Thanos Sepolia Live Playwright Verification)

- 실서비스 URL(`https://sentinai.tokamak.network/thanos-sepolia`)에 대해 Playwright MCP 기반 인터랙션 검증을 수행했다.
- 읽기 대시보드 API(`/thanos-sepolia/api/*`)는 정상 응답(`200`)이지만, 자율성 기능 API가 루트(`/api/*`)로 호출되어 `404`가 반복되는 것을 확인했다.
- 기능 단위 결과를 `PASS/FAIL`로 정리하고 증빙 파일(`스크린샷 2건`, `네트워크 요청 로그`)을 생성했다.
- 검증 리포트 문서화: `docs/verification/thanos-sepolia-live-playwright-verification-2026-03-04.md`.

---

## Detailed Review (2026-03-04 Dashboard Runtime Data Reliability Fix)

- `AutonomyPipeline` 클라이언트 훅의 API base path 하드코딩(`''`)을 제거하고 `NEXT_PUBLIC_BASE_PATH`를 사용하도록 수정해 `/thanos-sepolia` 배포 경로에서 `/api/* 404` 반복을 차단했다.
- `GET /api/metrics`에서 L2 RPC를 `L2_RPC_URL` 외에 `SENTINAI_L2_RPC_URL`, `NEXT_PUBLIC_L2_RPC_URL`까지 순차 fallback 하도록 확장했다.
- L2 RPC 미설정 fallback 경로에서도 pod 목록/리소스 기반 비용 데이터를 유지하도록 응답을 보강해 monthly cost가 `$0 / NaN`으로 무너지는 문제를 방지했다.
- 비용 카드 렌더링에 `fixedCost<=0` 방어를 추가해 NaN 퍼센트 표시를 제거했다.
