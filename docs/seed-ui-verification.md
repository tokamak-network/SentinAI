...
```bash
# Rising 시나리오 주입
curl -s -X POST "http://localhost:3001/api/metrics/seed?scenario=rising" | python3 -m json.tool
```
...
```bash
curl -s "http://localhost:3001/api/scaler" | python3 -c "
import sys, json
...
for scenario in stable rising spike falling; do
  echo "=== $scenario ==="
  curl -s -X POST "http://localhost:3001/api/metrics/seed?scenario=$scenario" > /dev/null
  sleep 1
  curl -s "http://localhost:3001/api/scaler" | python3 -c "
import sys, json
d = json.load(sys.stdin)
