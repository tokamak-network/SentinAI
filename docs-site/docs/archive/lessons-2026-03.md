...
## 2026-03-04 (Live Verification Addendum)

- 네트워크 prefix가 있는 라우트(`/thanos-sepolia`)에서 일부 프론트엔드 API 호출만 루트(`/api`)로 남아 있으면 대시보드는 부분적으로 살아 있어도 자율성 기능은 전면 실패한다.
- Rule: For prefixed network dashboards, all client API calls (including autonomy/seed/policy endpoints) must be generated from a single base-path resolver and covered by one E2E assertion that forbids root `/api/*` calls on prefixed pages.
- L2 RPC 미설정 fallback이 임의 블록값/0원 비용을 반환하면 운영자는 실제 장애를 데이터 오류로 인식한다.
- Rule: Metrics fallback responses must preserve observable reality (known last block + discovered components + derived cost estimate) and avoid synthetic random block numbers in production-facing dashboards.
