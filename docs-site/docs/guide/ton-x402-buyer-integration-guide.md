...
- 현재 TON Phase 1에서는 `merchant == relayer == spender`가 실체인에서 검증된 운영 전제다.
...
4. Buyer -> confirm merchant and spender are the same operator-controlled address in current TON Phase 1
5. Buyer -> approve(spender, amount) on TON ERC-20 if allowance is insufficient
6. Buyer -> sign EIP-712 PaymentAuthorization
7. Buyer -> retry GET with X-PAYMENT header
8. SentinAI -> facilitator settlement + paid response
9. Buyer -> verify receipt and optionally confirm settlement status / txHash
```
...
현재 TON Phase 1에서는 아래도 확인해야 한다.

- `merchant == facilitator.spender`
- `authorization.domain.verifyingContract == facilitator.spender`
...
- 현재 TON Phase 1에서는 이 spender 주소가 최종 merchant 수취 주소이기도 하다
